...
:::{#glossary-casual-user}
Casual User
: A user who does not have an annual or recurring membership. for the purpose of this case study only the term `Customer` is used. See [Customer](#glossary-customer)
:::
...
:A Common Table Expression (CTE) is a temporary named result set defined within a SQL query using the WITH keyword. CTEs make complex queries easier to read and maintain by allowing intermediate results to be referenced by name as if they were tables.

CTEs can be:

 -  Non-recursive: Used like a simple, named subquery.
 -  Recursive: Used to perform iterative operations such as generating sequences or traversing hierarchical data.
:::
...
:::{#glossary-docks}
Docks
:   The physical locking points at a bike share station where bicycles are securely parked and rented from. The number of docks at a station indicates its capacity (number of bikes it can hold at one time.)
:::
...
:::{#glossary-kumu}
Kumu
:   A web-based platform for visualizing complex systems and relationships. Kumu allows users to create interactive, dynamic maps that represent elements and the connections between them. In this project, it is used to illustrate data provenance by mapping how datasets, transformations, and outputs relate to one another.
:::
...
:::{#glossary-member}
Member
:  A user who has an annual or recurring membership. For the purpose of this case study only the term `Subscriber` is used.  See [Subscriber](#glossary-subscriber)  
:::

:::{#glossary-near-duplicate}
Near Duplicate
:  A stations who's name or coordinates are very close to that of another station, but no so close one can rule them as true duplicates.
:::

:::{#glossary-non-loop-ride}
Non-Loop Ride
:  A Non-Loop Ride refers to a ride that starts and ends at different stations. These rides represent purposeful travel between distinct stations, rather than [loop Rides](#glossary-loop-ride) that begin and end at the same point. In this case study, non-loop rides are analyzed separately from loop rides to better understand directional travel patterns and station-to-station dynamics. 
:::
...
:::{#glossary-unused-stations}
Unused Station
:   A station that is not the `start_station` or `end_station` of any ride in the rides database.
:::
...
:::{#glossary-week-part}
Week Part
:   A categorical variable indicating whether a given timestamp falls on a Weekday or Weekend. Derived from the day of the week of a ride’s start_time. Monday through Friday are classified as **Weekday**. Saturday and Sunday are classified as **Weekend**.
:::
